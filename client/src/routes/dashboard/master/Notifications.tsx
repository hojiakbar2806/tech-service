...
export default function NotificationsPage() {
    ...
    const handleMarkAsRead = () => {
        markAsRead.mutate(notifications.filter((notif) => !notif.seen).map((notif) => notif.id))
        refetch()
    }
        ...
        <div className="space-y-4 px-4">
            <div className="flex justify-between items-center">
                ...
                {notifications.some((notif) => !notif.seen) && (
                    <Button onClick={handleMarkAsRead}>O'qilgan deb belgilash</Button>
                )}
            </div>
            ...
            <div className="flex flex-col gap-2">
                {notifications.map((notif, i) => (
                    <div data-seen={notif.seen} key={i} className="flex flex-col gap-1 border shadow-md p-4 rounded-lg
                        data-[seen=true]:opacity-50">
                        <div className="flex justify-between items-center cursor-pointer">
                            ...
                            <Button
                                ...
                                className="cursor-pointer" variant={"ghost"}>O'qilgan deb belgilash</Button>
                        </div>
